...
export default async function handler(req,res){
  ...
  const normalize=str=>str.replace(/[٠-٩]/g,d=>"٠١٢٣٤٥٦٧٨٩".indexOf(d)).trim();
  ...
  try{
    ...
    if(type==="nationalId"){
      ...
      matches=rows.filter(r=>normalize(r[2]||"").replace(/\s/g,"").toUpperCase().includes(identifier.toUpperCase())); // افترضنا جواز السفر في العمود 3
    }
